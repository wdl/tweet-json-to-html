...
module.exports = async (tweet, bg = 'default') => {
    // 
    const mainTweet = tweet.data;
    const author = tweet.includes.users.find(o => o.id === mainTweet.author_id);
    const media = mainTweet.attachments?.media_keys.map(o => tweet.includes.media.find(p => p.media_key === o));
    const photo = media?.filter(o => o.type === 'photo');
    const video = media?.filter(o => o.type === 'video');
    // const poll = tweet.includes.referenced_tweets.map(o => tweet.includes.tweets.find(p => p.id === o.id));
    // const place = tweet.includes.referenced_tweets.map(o => tweet.includes.tweets.find(p => p.id === o.id));
    const referencedTweets = tweet.includes.referenced_tweets?.map(o => tweet.includes.tweets.find(p => p.id === o.id));

    // Localization settings
    const lang = mainTweet.lang;
    moment.locale(lang);
    ...
    const text = mainTweet.text.replace(/ https?:\/\/t.co\/[a-zA-Z0-9]*$/, '');
    const text_tagged = text.replace(/(https?:\/\/)([^\s]+)/g, '<a href="$&" target="_blank">$2</a>')
        ...
        .replace(/\n/g, '<br />');
    const link = `https://twitter.com/${author.username}/status/${mainTweet.id}`;
    ...
    if (photo) {
        ...
        const img_cnt = Math.min(photo.length, 4);
        ...
        for (let i = 1; i <= img_cnt; i++) {
            include_img = include_img.replace(`%IMAGE_URL_${i}%`, photo[i - 1].url);
        }

        const html_include_img = include_form.replace('%INCLUDE%', include_img);
        mainIncludes.push(html_include_img);
    }
    ...
    if (video) {
        ...
        include_video = include_video.replace(`%PREVIEW_IMAGE_URL%`, video[0].preview_image_url);
        include_video = include_video.replace('%PLAY%', play);

        const html_include_video = include_form.replace('%INCLUDE%', include_video);
        mainIncludes.push(html_include_video);
    }
